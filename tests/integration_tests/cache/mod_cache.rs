...
async fn lru_sampling_correctness_when_no_ttls() {
    ...
    assert!(
        !(d2_present && d3_present),
        "At least one of the two LRU candidates should be evicted by sampling"
    );
}
...
async fn batching_and_locking_under_pressure() {
    ...
    // Fill to capacity
    let docs: Vec<_> =
        (0..5).map(|i| Document::new(doc! {"k": i}, DocumentType::Persistent)).collect();
    for d in &docs {
    ...
    let t1 = tokio::spawn(async move {
        for i in 100..110 {
            cache1.insert(Document::new(doc! {"k": i}, DocumentType::Persistent));
        }
    });
    let t2 = tokio::spawn(async move {
        for i in 200..210 {
            cache2.insert(Document::new(doc! {"k": i}, DocumentType::Persistent));
        }
    });
...
async fn lfu_vs_lru_victim_selection_differs() {
    ...
    assert!(
        cache_hybrid.get(&c_h.id).is_none(),
        "Hybrid (LFU) should evict lower-frequency C among the tail"
    );
    assert!(
        cache_hybrid.get(&b_h.id).is_some(),
        "Hybrid should keep higher-frequency B even if older than C"
    );
}
