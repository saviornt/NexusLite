...
use nexus_lite::types::{DocumentId, Operation, SerializableBsonDocument, SerializableDateTime};
...
fn test_serializable_bson_document_serde() {
	...
	let (decoded, _): (SerializableBsonDocument, usize) =
...
fn test_serializable_datetime_serde() {
	...
	let (decoded, _): (SerializableDateTime, usize) =
		bincode::serde::decode_from_slice(&encoded, standard()).unwrap();
