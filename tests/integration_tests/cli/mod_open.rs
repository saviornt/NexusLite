...
fn test_open_pbe_env_missing() {
    ...
    unsafe {
        std::env::set_var("NEXUSLITE_PASSWORD", "secret");
    }
    nexuslite::api::encrypt_db_with_password(db_path.as_path(), "user", "secret").unwrap();
    unsafe {
        std::env::remove_var("NEXUSLITE_PASSWORD");
    }

    // Now try to open via CLI programmatic without env; since tests run non-interactively, this should error
    let r = run(&engine, Command::DbOpen { db_path: db_path.clone() });
    assert!(r.is_err());

    // Provide env and try again
    unsafe {
        std::env::set_var("NEXUSLITE_USERNAME", "user");
    }
    unsafe {
        std::env::set_var("NEXUSLITE_PASSWORD", "secret");
    }
