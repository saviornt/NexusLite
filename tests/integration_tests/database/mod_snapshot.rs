...
use nexuslite::{Database, index::IndexKind};
...
fn snapshot_newer_version_errors_gracefully() {
    ...
    use nexuslite::wasp::{DbSnapshot, SNAPSHOT_CURRENT_VERSION, SnapshotFile};
    ...
    let err = nexuslite::wasp::decode_snapshot_from_bytes(&bytes).unwrap_err();
