...
use nexuslite::types::DocumentId;
use std::fs;
...
fn test_planner_uses_index_equality_stats_hit() {
    ...
    for i in 0..50i32 {
        col.insert_document(nexuslite::document::Document::new(
            doc! {"k": i},
    ...
    let mut hits = 0u64;
    {
        ...
        if let Some(idx) = mgr.indexes.get_mut("k") {
            match idx {
                nexuslite::index::IndexImpl::Hash(h) => hits = h.stats.hits,
                nexuslite::index::IndexImpl::BTree(b) => hits = b.stats.hits,
            }
        }
    }
    assert!(hits > 0);
...
fn test_collection_index_invalidation_update_delete() {
    ...
    let d2 = nexuslite::document::Document::new(
        doc! {"k": 7, "v": "x"},
        nexuslite::document::DocumentType::Persistent,
    );
...
fn test_index_metadata_version_bump() {
    with_env_lock(|| {
        ...
        assert_eq!(
            ...
            nexuslite::index::INDEX_METADATA_VERSION
        );
    });
}
...
fn test_index_wasp_overlay_compensates_index_miss() {
    with_env_lock(|| {
        ...
        let doc = nexuslite::document::Document::new(
            doc! {"k": 42, "v": "x"},
            nexuslite::document::DocumentType::Persistent,
        );
        let id = doc.id.clone();
...
fn test_index_wasp_overlay_persists_across_restart() {
    with_env_lock(|| {
        ...
        assert!(deltas.iter().any(|d| d.collection == col_name
            && d.field == "k"
            && matches!(d.op, nexuslite::wasp::DeltaOp::Add)
            && d.id == id));
    });
}
...
fn test_index_wasp_overlay_across_restart_compensates_index_miss() {
    with_env_lock(|| {
        ...
        let engine2 = Engine::with_wasp(wasp_path.clone()).expect("WASP engine should re-open");
        let col2 = engine2.create_collection(col_name.into());
        let mut doc2 = nexuslite::document::Document::new(
            data.clone(),
            nexuslite::document::DocumentType::Persistent,
        );
        doc2.id = id.clone();
