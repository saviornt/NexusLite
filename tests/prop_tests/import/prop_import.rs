...
proptest! {
    ...
    let engine = Engine::new(dir.path().join("prop_import.wasp")).unwrap();
    ...
    let rep = nexuslite::import::import_from_reader(&engine, cur, nexuslite::import::ImportFormat::Csv, &opts).unwrap();
