...
async fn test_cli_import_export_ndjson() {
    ...
    run(
        &engine,
        Command::Import { collection: "users".into(), file: in_path.clone(), format: Some("ndjson".into()) },
    )
    .unwrap();
    ...
    run(
        &engine,
        Command::Export { collection: "users".into(), file: out_path.clone(), format: Some("ndjson".into()) },
    )
    .unwrap();
