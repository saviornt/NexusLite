...
fn cli_crypto_encrypt_decrypt_and_sign_verify() {
    ...
    run(&engine, Command::CryptoKeygenP256 { out_priv: Some(priv_p.clone()), out_pub: Some(pub_p.clone()) }).unwrap();
    // Encrypt/Decrypt
    let enc = dir.path().join("msg.enc");
    let dec = dir.path().join("msg.dec");
    run(&engine, Command::CryptoEncryptFile { key_pub: pub_p.clone(), input: input.clone(), output: enc.clone() }).unwrap();
    run(&engine, Command::CryptoDecryptFile { key_priv: priv_p.clone(), input: enc.clone(), output: dec.clone() }).unwrap();
    let dec_bytes = fs::read(&dec).unwrap();
    assert_eq!(dec_bytes, b"hello world");
    // Sign/Verify
    let sig = dir.path().join("sig.der");
    run(&engine, Command::CryptoSignFile { key_priv: priv_p.clone(), input: input.clone(), out_sig: Some(sig.clone()) }).unwrap();
    run(&engine, Command::CryptoVerifyFile { key_pub: pub_p.clone(), input: input.clone(), sig: sig.clone() }).unwrap();
}
...
fn cli_encrypted_checkpoint_roundtrip() {
    ...
    run(&engine, Command::CheckpointEncrypted { db_path: db_path.clone(), key_pub: pub_p.clone(), output: enc_snap.clone() }).unwrap();
    // Restore into a fresh path
    let db_restored = dir.path().join("restored.db");
    run(&engine, Command::RestoreEncrypted { db_path: db_restored.clone(), key_priv: priv_p.clone(), input: enc_snap.clone() }).unwrap();
