...
fuzz_target!(|data: &[u8]| {
    ...
    let engine = match nexuslite::engine::Engine::new(std::env::temp_dir().join("fuzz_ndjson.wasp")) { Ok(e) => e, Err(_) => return };
    let mut opts = nexuslite::import::ImportOptions::default();
    opts.collection = "fuzz".into();
    let cur = Cursor::new(s.as_bytes());
    let _ = nexuslite::import::import_from_reader(&engine, cur, nexuslite::import::ImportFormat::Ndjson, &opts);
});
