...
// The WASP recovery engine from the `/src/recovery` directory
...
use bson::doc;
use nexuslite::cache::CacheConfig;
...
async fn main() {
    ...
    let col_wal = engine_wal.create_collection_with_config(
        "bench".into(),
        CacheConfig { capacity: n + 16, ..Default::default() },
    );
    let start_insert_wal = Instant::now();
    for i in 0..n {
        let d = Document::new(
            doc! {"i": i as i64, "payload": format!("x{i}")},
    ...
    let col_wasp = engine_wasp.create_collection_with_config(
        "bench".into(),
        CacheConfig { capacity: n + 16, ..Default::default() },
    );
