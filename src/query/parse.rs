...
use super::types::{CmpOp, Filter, UpdateDoc, MAX_IN_SET};
...
impl TryFrom<UpdateDocSerde> for UpdateDoc {
    ...
    fn try_from(us: UpdateDocSerde) -> Result<Self, Self::Error> {
        ...
        if let Some(setd) = us.set {
            for (k, v) in setd.into_iter().take(128) { out.set.push((k, v)); }
        }
        if let Some(incd) = us.inc {
            for (k, v) in incd.into_iter().take(128) {
                #[allow(clippy::cast_precision_loss)]
                let f = match v { Bson::Int32(i) => f64::from(i), Bson::Int64(i) => i as f64, Bson::Double(d) => d, _ => { return Err(DbError::QueryError("$inc requires numeric".into())); } };
                out.inc.push((k, f));
            }
        }
