...
pub enum IndexKeyKind { Str(String), F64(OrderedFloat<f64>), I64(i64), Bool(bool) }
...
impl Hash for EqKey {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match &self.0 {
            IndexKeyKind::Str(s) => { 0u8.hash(state); s.hash(state); }
            IndexKeyKind::F64(f) => { 1u8.hash(state); f.hash(state); }
            IndexKeyKind::I64(i) => { 2u8.hash(state); i.hash(state); }
            IndexKeyKind::Bool(b) => { 3u8.hash(state); b.hash(state); }
        }
    }
}
...
fn get_path<'a>(doc: &'a BsonDocument, path: &str) -> Option<&'a Bson> {
    ...
    for p in parts {
        match cur { Bson::Document(d) => { cur = d.get(p)?; }, _ => return None }
    }
    Some(cur)
}
...
impl BTreeIndex {
    ...
    pub fn lookup_range(&mut self, min: Option<&Bson>, max: Option<&Bson>, inclusive_min: bool, inclusive_max: bool) -> Option<Vec<DocumentId>> {
        ...
        let iter: Box<dyn Iterator<Item = (&OrdKey, &BTreeSet<DocumentId>)>> = match (start.as_ref(), end.as_ref()) {
            (Some(s), Some(e)) => {
                if inclusive_min && inclusive_max { Box::new(self.map.range(s.clone()..=e.clone())) }
                else if inclusive_min { Box::new(self.map.range(s.clone()..e.clone())) }
                else if inclusive_max { Box::new(self.map.range((std::ops::Bound::Excluded(s.clone()), std::ops::Bound::Included(e.clone())))) }
                else { Box::new(self.map.range((std::ops::Bound::Excluded(s.clone()), std::ops::Bound::Excluded(e.clone())))) }
            }
            (Some(s), None) => { if inclusive_min { Box::new(self.map.range(s.clone()..)) } else { Box::new(self.map.range((std::ops::Bound::Excluded(s.clone()), std::ops::Bound::Unbounded))) } }
            (None, Some(e)) => { if inclusive_max { Box::new(self.map.range(..=e.clone())) } else { Box::new(self.map.range(..e.clone())) } }
            (None, None) => Box::new(self.map.iter()),
        };
        for (_k, set) in iter { out.extend(set.iter().cloned()); }
        if out.is_empty() { self.stats.misses += 1; None } else { self.stats.hits += 1; Some(out) }
    }
}
...
pub struct IndexDescriptor { pub field: String, pub kind: IndexKind }
...
pub fn index_insert_all(mgr: &mut IndexManager, doc: &BsonDocument, id: &DocumentId) {
    for idx in mgr.indexes.values_mut() {
        match idx { IndexImpl::Hash(h) => h.insert(doc, id), IndexImpl::BTree(b) => b.insert(doc, id) }
    }
}

pub fn index_remove_all(mgr: &mut IndexManager, doc: &BsonDocument, id: &DocumentId) {
    for idx in mgr.indexes.values_mut() {
        match idx { IndexImpl::Hash(h) => h.remove(doc, id), IndexImpl::BTree(b) => b.remove(doc, id) }
    }
}
...
pub fn lookup_range(mgr: &mut IndexManager, field: &str, min: Option<&Bson>, max: Option<&Bson>, incl_min: bool, incl_max: bool) -> Option<Vec<DocumentId>> {
    match mgr.indexes.get_mut(field) {
