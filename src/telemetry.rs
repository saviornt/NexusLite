...
impl Default for TelemetryConfig {
    fn default() -> Self {
        let slow = std::env::var("NEXUS_SLOW_QUERY_MS").ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(500);
    Self { slow_query_ms: slow, query_log_path: None, structured_json: true, enable_audit: false, max_result_limit: 10_000, current_db: None, per_collection_max: HashMap::new() }
    }
}
...
pub fn set_query_log(path: PathBuf, slow_query_ms: Option<u64>, structured_json: Option<bool>) {
    ...
    if let Some(ms) = slow_query_ms { w.slow_query_ms = ms; }
    if let Some(js) = structured_json { w.structured_json = js; }
}
pub fn set_slow_query_ms(ms: u64) { TELEMETRY.cfg.write().slow_query_ms = ms; }
pub fn set_audit_enabled(enabled: bool) { TELEMETRY.cfg.write().enable_audit = enabled; }
pub fn set_audit_sink_for_tests(sink: Arc<RwLock<Vec<String>>>) { *TELEMETRY.audit_sink.write() = Some(sink); }
...
fn now_ts() -> String { chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Millis, true) }
...
pub fn log_query(collection: &str, filter_dbg: &str, duration_ms: u128, limit: Option<usize>, skip: Option<usize>, user: Option<&str>) {
    ...
    if let Some(path) = cfg.query_log_path.as_ref() {
        if cfg.structured_json {
            ...
            let line = format!("ts={} db={} collection={} filter_hash={} duration_ms={} limit={:?} skip={:?} user={:?} slow={}", now_ts(), cfg.current_db.as_deref().unwrap_or("default"), collection, filter_hash, duration_ms, limit, skip, user, slow);
            write_line(path, &line);
        }
    }
}
...
pub fn log_audit(op: &str, collection: &str, doc_id: &str, user: Option<&str>) {
    ...
    if !TELEMETRY.cfg.read().enable_audit { return; }
...
pub fn max_result_limit() -> usize { TELEMETRY.cfg.read().max_result_limit }

pub fn set_max_result_limit_global(limit: usize) { TELEMETRY.cfg.write().max_result_limit = limit; }
...
pub fn remove_rate_limit(collection: &str) { TELEMETRY.rate_limits.write().remove(collection); }
