...
static FLAGS: LazyLock<RwLock<HashMap<String, FeatureFlag>>> = LazyLock::new(|| {
    ...
    map.insert(
        ...
        FeatureFlag {
            ...
            enabled: true,
            description: "Expose minimal OpenMetrics-style counters via telemetry::metrics_text().".to_string(),
        },
    );
    map.insert(
        "regex".to_string(),
        FeatureFlag {
            ...
            description: "Regular expression operators in queries (requires Cargo feature 'regex').".to_string(),
        },
    );
    map.insert(
        "cli-bin".to_string(),
...
pub fn set(name: &str, enabled: bool) -> bool {
    ...
    if let Some(f) = g.get_mut(name) { f.enabled = enabled; true } else { false }
}
