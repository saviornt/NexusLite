...
impl CowTree {
    ...
    pub fn insert(&mut self, key: Vec<u8>, value: Vec<u8>) -> io::Result<()> {
        ...
        fn insert_rec(
            ...
            let mut node = {
                ...
                decode_from_slice::<CowNode, _>(&page.data, standard())
                    .map(|(n, _)| n)
                    .unwrap_or(CowNode::new_leaf())
            };
            match &mut node {
                ...
                CowNode::Internal { keys, children } => {
                    ...
                    if let Some((promo_key, right_id)) = promoted {
                        ...
                        if keys.len() > MAX_KEYS {
                            ...
                            let Some(promoted) = keys.pop() else {
                                return Err(io::Error::other("internal split with empty keys"));
                            };
                            let right =
                                CowNode::Internal { keys: right_keys, children: right_children };
                            let new_right_id = tree.alloc.alloc();
                            let right_bytes = encode_to_vec(&right, standard())
                                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
                            let right_page =
                                Page::new(new_right_id, tree.version + 1, 2, right_bytes);
                            tree.write_page(new_right_id, &right_page)?;
                            let left_bytes = encode_to_vec(&node, standard())
                                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
                            let new_left_id = tree.alloc.alloc();
        ...
        if let Some((promo_key, right_id)) = promoted {
            let new_root =
                CowNode::Internal { keys: vec![promo_key], children: vec![new_root_id, right_id] };
    ...
    fn write_page(&mut self, page_id: u64, page: &Page) -> io::Result<()> {
        let offset = 2 * crate::utils::num::usize_to_u64(WASP_PAGE_SIZE)
            + (page_id - 1) * crate::utils::num::usize_to_u64(WASP_PAGE_SIZE);
    ...
    let required_size = offset + crate::utils::num::usize_to_u64(WASP_PAGE_SIZE);
    ...
    fn read_page(&mut self, page_id: u64) -> io::Result<Page> {
        let offset = 2 * crate::utils::num::usize_to_u64(WASP_PAGE_SIZE)
            + (page_id - 1) * crate::utils::num::usize_to_u64(WASP_PAGE_SIZE);
        let file_size = self.file.file.metadata()?.len();
    if file_size < offset + crate::utils::num::usize_to_u64(WASP_PAGE_SIZE) {
            return Err(io::Error::new(
                io::ErrorKind::UnexpectedEof,
                format!(
                    "File too small: size={}, need at least {} for page_id {} at offset {}",
                    file_size,
                    offset + crate::utils::num::usize_to_u64(WASP_PAGE_SIZE),
                    page_id,
                    offset
                ),
            ));
        ...
        let (page, _): (Page, _) = decode_from_slice(&buf, standard())
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        Ok(page)
    }
}
...
impl CowTree {
    ...
    pub fn recover_from_wal(&mut self, wal: &mut super::wal::TinyWal) -> io::Result<()> {
        ...
        if let Some(last) = recs.last() {
            ...
            m.root_page_id = self.root_page_id;
