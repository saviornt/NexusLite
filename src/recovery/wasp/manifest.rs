...
impl WaspFile {
    ...
    let manifest_offsets = [0, crate::utils::num::usize_to_u64(WASP_PAGE_SIZE)];
    ...
    // Compute next slot without premature narrowing; 2 slots only.
    let next_slot_u64 = (self.manifest_version + 1) % 2;
    let next_slot = crate::utils::num::u64_to_usize(next_slot_u64).unwrap_or(0);
    ...
    pub fn read_manifest(&mut self) -> io::Result<Manifest> {
        ...
        for &offset in &self.manifest_offsets {
            ...
            let (page, _): (Page, _) = decode_from_slice::<Page, _>(&buf, standard())
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
            if page.header.page_type == 1
                && page.verify_crc()
        ...
        match best {
            ...
            None => Err(io::Error::new(
                ...
                "Shadow paging: no valid manifest found",
            )),
        }
    }
}
