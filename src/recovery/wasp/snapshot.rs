...
pub fn encode_snapshot_file(snap: &DbSnapshot) -> io::Result<Vec<u8>> {
    ...
    encode_to_vec(&file, standard()).map_err(|e| {
        io::Error::new(io::ErrorKind::InvalidData, format!("snapshot encode failed: {e}"))
    })
}
...
pub fn decode_snapshot_from_bytes(bytes: &[u8]) -> io::Result<DbSnapshot> {
    if bytes.len() < 4 || bytes[0..4] != SNAPSHOT_MAGIC {
        return Err(io::Error::new(
            ...
            "snapshot decode: missing or invalid magic",
        ));
    }
    let (file, _) = decode_from_slice::<SnapshotFile, _>(bytes, standard()).map_err(|e| {
        io::Error::new(io::ErrorKind::InvalidData, format!("snapshot decode failed: {e}"))
    })?;
    if file.version > SNAPSHOT_CURRENT_VERSION {
        return Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "snapshot decode: version newer than this build supports",
        ));
    }
    Ok(file.snapshot)
}
